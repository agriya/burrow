...
// rajesh_059a509 // 2010-10-04 / compatibility hack taken from http://pear.php.net/bugs/bug.php?id=16521&edit=3
require_once CORE_PATH . 'vendors' . DS . 'PEAR' . DS . 'PHP' . DS . 'Compat' . DS . 'Function' . DS .'hash.php';
require_once CORE_PATH . 'vendors' . DS . 'PEAR' . DS . 'PHP' . DS . 'Compat' . DS . 'Function' . DS .'hash_hmac.php';
//<--
...
class WindowsLiveLogin
{
    ...
    private function derive($secret, $prefix)
    {
        ...
        $keyLen = 16;
        $key = $prefix . $secret;
        // rajesh_059at09 / 2010-10-04 / removed mhash dependency
        $key = hash('sha256', $key, true); // Was: $key = mhash(MHASH_SHA256, $key);
